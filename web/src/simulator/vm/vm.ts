...
export class Vm {
    ...
    for (const fn of Object.values(this.functionMap)) {
      for (const inst of fn.operations) {
        if (inst.op == "pop" || inst.op == "push") {
          ...
          if (isErr(base)) {
            return Err(createError((base as ErrResult<Error>).error.message, inst.span));
          }
        }
      }
    }
      ...
      if (!functions.has(call.name)) {
        ...
        if (builtin) {
          ...
          if (expectedNArgs != call.nArgs) {
            return Err(
              createError(
                `OS function ${call.name} expects ${expectedNArgs
  ...
  get invocation(): VmFunctionInvocation {
    const invocation = this.executionStack[this.executionStack.length - 1];
