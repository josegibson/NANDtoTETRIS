...
class SymbolTable:
    ...
    def startSubroutine(self, name, kind, type):
    ...
    def define(self, name, type, kind):
        entry = {'kind': kind, 'type': type, 'index': self.index_counters[kind]}

        self.index_counters[entry['kind']] += 1

        scope = None
        if entry['kind'] in ['static', 'field']  and name in self.class_scope:
            raise ValueError(f'Symbol defined more that once in the class scope: {name}')
        elif entry['kind'] in ['var', 'arg'] and name in self.subroutine_scope:
            raise ValueError(f'Symbol defined more that once in the subroutine scope: {name}')
        elif entry['kind'] not in ['static', 'field', 'var', 'arg']:
